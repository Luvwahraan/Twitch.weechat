...
def update_request(irc_server):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Can't refresh toker, HTTP error: {response.status_code}")
...
def addserver_command_cb(data, buffer, args):
    ...
    if len(args_list) > 2:
        ...
        weechat.prnt('', f"[{SCRIPT_NAME}] /{SCRIPT_NAME} servername refresh_token")
        return weechat.WEECHAT_RC_OK
...
if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, 
    ...
    weechat.hook_command( SCRIPT_NAME,
            'Add update token for a twitch server.', 
            '', '', '',
